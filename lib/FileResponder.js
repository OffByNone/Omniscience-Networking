...
class FileResponder {
    ...
    sendResponse(request, filePath) {
		let file = this._fileUtils.create(filePath);
		this._fileUtils.readBytes(file.path).then(fileBytes => {
			...
			let responseHeaders = this._responseBuilder.createResponseHeaders(request.headers, file, fileResponseBytes.byteLength);
